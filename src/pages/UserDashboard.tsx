...
import { ReservationDepositNotice } from '../components/ReservationDepositNotice';

import { PendingReservationModal } from '../components/PendingReservationModal';
...
export function UserDashboard() {
  ...
  const navState = (location.state as DashboardNavState | null) ?? {};

  const { t } = useLanguage();

  const { isAuthenticated, role } = useAuth();

  const [createdBannerContractId] = useState(() =>
    navState.reservationCreated ? navState.contractId : undefined
  );
  ...
  useEffect(() => {
    if (!contracts?.length || !createdBannerContractId) return;
    const match = contracts.find((c) => c.id === createdBannerContractId);
    if (match?.state === 'DRAFT') {
      setSelectedContract(match);
    }
  }, [contracts, createdBannerContractId]);
  ...
  const handleCancelSelected = () => {
    ...
    if (confirm(t('confirmCancelReservationUser'))) {
      ...
      cancelMutation.mutate(selectedContract.id);
  ...
  return (
    ...
    <Layout>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {createdBannerContractId != null && (
          <ReservationDepositNotice
            variant="success"
            className="mb-6"
      ...
      <PendingReservationModal
        ...
        isOpen={!!selectedContract && selectedContract.state === 'DRAFT'}

        onClose={() => setSelectedContract(null)}

        onCancel={selectedContract?.state === 'DRAFT' ? handleCancelSelected : undefined}
...
function ContractCard({
  ...
  const isPending = contract.state === 'DRAFT';

  const needsDeposit = isPending && !contract.depositConfirmed;
  ...
  return (
    ...
    <Card
      className={`p-6 ${isPending ? 'ring-1 ring-amber-100' : ''}`}
